...
Shows individual model predictions and their average.
"""
import torch
...
def visualize_ensemble():
    ...
    # Select samples: 15 best, 10 random, 25 worst
    sorted_idxs = np.argsort(ensemble_errors)
    best_idxs = sorted_idxs[:15]
    worst_idxs = sorted_idxs[-25:]
    remaining = sorted_idxs[15:-25]
    random_idxs = np.random.choice(remaining, min(10, len(remaining)), replace=False) if len(remaining) > 0 else []
    
    selected = np.concatenate([best_idxs, random_idxs, worst_idxs])
    labels_text = ["Best"] * len(best_idxs) + ["Random"] * len(random_idxs) + ["Worst"] * len(worst_idxs)
    ...
    for i, idx_local in enumerate(selected):
        row = (i // cols) * 2
        col = i % cols
        ...
        color = 'green' if error_m < 10 else 'orange' if error_m < 20 else 'red'
        ax_img.set_title(f"{labels_text[i]}: {error_m:.1f}m", color=color, fontweight='bold', fontsize=11)
