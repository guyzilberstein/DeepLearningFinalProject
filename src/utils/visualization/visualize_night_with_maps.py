"""
Visualize night predictions with maps.
...
def visualize_night_with_maps(num_per_category=7):
    """Create night predictions visualization with maps."""
    night_csv = os.path.join(project_root, 'data', 'metadata', 'night_holdout.csv')
    ...
    print(f"Running inference on night set ({len(night_dataset)} samples)...")
    predictions, labels = get_ensemble_predictions(night_loader, device)
    ...
    print(f"Plotting {num_samples} night samples...")
    ...
    for i, (idx, category, cat_color) in enumerate(zip(selected, categories, colors_cat)):
        ...
        img_name = night_dataset.data_frame.iloc[idx]['filename']
    ...
    fig.suptitle(f'Night Image Predictions Analysis\n'
                 f'Mean Error: {mean_error:.2f}m | Median: {median_error:.2f}m | {len(night_dataset)} samples',
    ...
    output_path = os.path.join(OUTPUT_DIR, 'night_predictions_analysis.png')
...
if __name__ == "__main__":
    ...
    visualize_night_with_maps(num_per_category=args.num)
